function [epdf,epdf_dp, edges, h, mid, loc] = histcn(X,xS, varargin)
...
loc = zeros(size(X));
sz = zeros(1,nd);
h  = zeros(1,nd); % add this for fixed bin width  --------------->  (added)
...
% This part is added to for sensitivity  ------------------------------>(added)
    [Ns,nPar] = size(xS.samp);
    epdf_dp = cell (nPar,4); 
    for kk = 1 : nPar
        epdf_dp {kk,1} = accumarray(loc(hasdata,:), xS.senA(: , kk), sz)/Ns/prod(h);
        epdf_dp {kk,2} = accumarray(loc(hasdata,:), xS.senB(: , kk), sz)/Ns/prod(h);
        epdf_dp {kk,3} = accumarray(loc(hasdata,:), xS.senC(: , kk), sz)/Ns/prod(h);
        epdf_dp {kk,4} = accumarray(loc(hasdata,:), xS.senD(: , kk), sz)/Ns/prod(h);
    end

    epdf = count/Ns/prod(h);  % estimated pdf
